...
import { useTheme, defaultTheme as noopTheme, Theme as DefaultTheme } from '../theme';
import { StylesContext } from './StylesProvider';
import { getStylesCreator } from './getStylesCreator';
import { increment } from './indexCounter';
import { Classes, ClassNameMap, Styles, WithStylesOptions } from './types';
...
function attach({ state, theme, stylesOptions, stylesCreator, name, }, props) {
    ...
    const { sheetsRegistry } = stylesOptions;
...
function useSynchronousEffect(func: () => Function, values: DependencyList) {
    const key = React.useRef([]);
    let output: Function | undefined;
...
export function makeStylesKit<
    Props extends object = {},
    Theme = DefaultTheme,
    ClassKey extends string = string
>(styles: Styles<Theme, Props, ClassKey>, options: Omit<WithStylesOptions<Theme>, 'withTheme'> = {}): keyof Props extends never
    ...
    // @ts-ignore
    const stylesCreator = getStylesCreator(styles);
    ...
    const useStyles = (props?: {classes?: Classes}) => {
        ...
        const shouldUpdate = React.useRef<any>();
