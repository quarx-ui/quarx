...
import {
    isEditingEndTime,
    isEditingStartTime,
} from '@core/components/experimental/DatePicker/components/Block/components/DatePickerRightSection/utils';
import { isValidTime } from '../../utils';
import { parseTimeParts } from '../FooterDatePicker/utils';
import { InnerTimeSetters } from '../../types';
...
export const getDateFromTimeByBadgeInPeriod = (
    ...
    if (isValidTime(hour, minutes, seconds)) {
        if (isEditingEndTime(selected, editablePartOfPeriod)) {
            ...
            return {
                ...selected,
                end: setHours(setMinutes(setSeconds(selected.end || new Date(), seconds), minutes), hour),
            };
        } if (isEditingStartTime(selected, editablePartOfPeriod)) {
            setTimes.setStartTime(timeWithSeconds);
            return {
                ...selected,
                start: setHours(setMinutes(setSeconds(selected.start || new Date(), seconds), minutes), hour),
            };
        }
    }
    return selected;
...
export const getDateFromTimeByBadgeInPicker = (
    { selected, newTime, setTimes }: SetTimeByBadgeInPicker,
): PickerSelectedDate => {
    const timeWithSeconds = `${newTime}:00`;
    const [hour, minutes, seconds] = parseTimeParts(timeWithSeconds);
    if (isValidTime(hour, minutes, seconds)) {
