...
export const getMask = (isSingleDate: boolean, withTime: DatePickerProps['withTime']) => getPlaceholder(isSingleDate, withTime).split('').filter((symbol) => symbol.match(/[ДМГЧ]/g));
