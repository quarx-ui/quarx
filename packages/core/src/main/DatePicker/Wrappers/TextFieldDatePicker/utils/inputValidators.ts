...
import { getFormatValue } from '@core/src/main/DatePicker/Wrappers/TextFieldDatePicker/utils/format';
...
type PickerTypeAndWithTime = Pick<ValidateJoinedDateProps, 'withTime' |'isPicker'>;
...
const mapFormatToFormatCells = ({ isPicker, withTime }: PickerTypeAndWithTime) => getFormatValue(isPicker, withTime).split(/[^#]/).filter(Boolean);
...
const mapJoinedInputToDateTimeStringParts = (
    joinedInput: string, { isPicker, withTime }: PickerTypeAndWithTime,
) => {
    const formatCells = mapFormatToFormatCells({ isPicker, withTime });
...
export const validateDateOnInput = (props: ValidateJoinedDateProps) => {
    const { isPicker, withTime, joinedInput } = props;
    const dateTimeStringParts = mapJoinedInputToDateTimeStringParts(joinedInput, { isPicker, withTime });
