...
> Для обновления тестов нужно добавить ` --update-snapshots` или `-u`
...
Но при необходимости данное поведение можно переопределить как для всех тестов, так и для конкретного (интерфейс не отличается). Указать нужные ключи можно следующим способом:

```ts
groupBy = ['testName', 'props', 'value', 'postfix'];
```

Передается массив ключей, которые будут включены в сортировку, в указанном порядке.
